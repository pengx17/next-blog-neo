...
最近一直在一个叫 Slock[1] 的工具里做开发，几乎很少打开 Codex 或者 Claude Code 了。
...
Slock 里的 Agent 不一样。每个 Agent 都有自己独立的工作区和记忆系统。它跟我们常用的那种 Slack-接-coding-CLI 类型的 broker（比如 Codex 3720[2] 那种）也不一样：那种 broker 可以并发分身，开多个会话；Slock 里的 Agent 不能分身——它在干一件事的时候，另外的活就得排队。


OpenClaw[3] 兴起来之后，大家慢慢把这种常驻 Agent 叫**龙虾**。一只龙虾有自己独立的工作目录、一套不断累积的记忆、不能同时做两件事。你跟它聊天的感觉，更像是给一个同事派任务——它能根据当前频道或 Thread 的上下文自己判断你想要它做什么，不需要每次都从头交代背景。
...
不过 multi-agent 也不是所有场景都帮上忙。最近做的一次 Onee-sama[4] 迁移就是个挺典型的反例。
...
<details>
<summary>脚注 / Footnotes</summary>

注


**Slock**：AI 原生的人 + AI Agent 协作平台。UI 类似 Slack（频道 / Thread / DM / @），但把 Agent 注册、调度、记忆都做进同一个界面。每个 Agent 是一个常驻进程，有自己的工作目录、独立记忆和身份。 ↩


**Codex 3720**（[slack-codex-broker](https://github.com/HOOLC/slack-codex-broker)）：把 coding CLI（如 Codex）接进 Slack 的一种 broker——人在 Slack 里 @ 它，它在后端 spawn 一个 Codex app-server thread 干活、汇报结果。每个 Slack thread 对应一个独立 workspace，跑完即弃，不积累长期身份。 ↩


**OpenClaw**：一个面向 agent workflow 的开发框架，"龙虾"这个术语就是从它的社区里慢慢叫开的——用来指代"独居、长记忆、不能分身"那类常驻 agent。 ↩


**Onee-sama**（[oneesama](https://github.com/afk-surf/oneesama)）：开源框架，把 AI 助理塞进 Google Meet 会议 + Slack 工作流。它本身是一个薄编排壳——audio / captions 收集 + 命令路由 + 权限管理——AI 大脑由你自己挂（Claude / Codex / 别的）。 ↩
